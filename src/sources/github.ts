...
function isDocsLikePath(path: string): boolean {
  ...
  if (
    ...
    return /\.(md|mdx|txt|rst|html?)$/i.test(lower);
  }
  return false;
}
...
export async function collectGithubFiles(
  input: string,
): Promise<{ rootUri: string; title: string; files: FolderFile[]; ref: string }> {
  ...
  try {
    ...
    const mapped = files.map((f) => {
      const rel = relative(tmp, f.path).split("\\").join("/");
      return {
        path: rel,
        uri: `${rootUri}/blob/${ref}/${rel}`,
        content: f.content,
      };
    });
    const filtered = mapped.filter((f) => isDocsLikePath(f.path));
    return {
      rootUri,
      title,
      ref,
      files: filtered.length > 0 ? filtered : mapped,
    };
  } finally {
...
export async function collectGithubViaApi(
  ...
  const paths = (tree.tree ?? [])
    .filter((t) => t.type === "blob" && isDocsLikePath(t.path) && (t.size ?? 0) < 1_000_000)
    .slice(0, 500);
  ...
  for (const item of paths) {
    ...
    const rawUrl = `https://raw.githubusercontent.com/${parsed.owner}/${parsed.repo}/${ref}/${item.path}`;
    ...
    files.push({
      path: item.path,
      uri: `${rootUri}/blob/${ref}/${item.path}`,
      content,
    });
  }
  return { rootUri, title, files, ref };
}
